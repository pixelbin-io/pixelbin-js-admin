...
const getQuery = (path) => {
    ...
    if (parseUrl.length > 1) {
        ...
        for (let item of searchParams) {
            ...
            queryResult[key] = queryResult[key] ? [...queryResult[key], value] : [value];
        }
        return queryResult;
    }
    return {};
            ...
            for (j = 0; j < linkArr.length; ++j) {
                if (linkArr[j].startsWith(":")) {
                    ...
                    if (bestMatch.params[linkArr[j].slice(1)]) {
                        ...
                        arrParams = [...bestMatch.params[linkArr[j].slice(1)], ...arrParams];
                    }
...
const getCorrectFileType = (fileType, fileExt) => {
    ...
    if (derivedContentType === "application/zip" && fileExt === "apk") {
        ...
        derivedContentType === "audio/x-ms-asf" &&
        derivedExt === "asf"
    ) {
        correctExt = fileExt;
    } else if (
        derivedContentType === "application/octet-stream" &&
        fileExt !== "bin"
    ) {
        correctExt = fileExt;
    }
 ...
 */
 const fileTypeFromBuffer = async (buffer, opts = {}) => {
