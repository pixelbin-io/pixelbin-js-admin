...
describe("UrlUtils test", () => {
  ...
  it("should get obj from url - error 2", async () => {
    ...
    const expectedObj = {
      ...
      version: "v1",
      filePath:
        "searchlight/platform-panel/settings/policy/faq/add-faq-group.png",
      worker: false,
      workerPath: "",
      options: {},
    };
    ...
    const expectedObj = {
      transformations: [
        ...
        { name: "extend", plugin: "t" },
        {
          plugin: "p",
          name: "apply",
          values: [{ key: "n", value: "presetNameXyx" }],
        },
      ],
      cloudName: "red-scene-95b6ea",
      zone: "z-slug",
  ...
  it("should throw error if transformation object is incorrect", async () => {
    ...
    expect(() => objToUrl(obj)).toThrowError(
      "value not specified for key 'h' in 'resize'",
    );
    obj.version = "v1";
    expect(() => objToUrl(obj)).toThrowError(
      "value not specified for key 'h' in 'resize'",
  ...
  it("should throw error if worker is true but workerPath is not defined", async () => {
    ...
    expect(() => objToUrl(obj)).toThrowError(
      "key workerPath should be a defined",
    );
  });
  ...
  it("should get failure while retrieving obj from url with invalid options", async () => {
    ...
    expect(() => urlToObj(optionsUrl)).toThrowError(
      PDKIllegalQueryParameterError,
    );
  });
  ...
  describe("custom domain", () => {
    it("should get obj from url - default zone with transformations - filedepth - 1", async () => {
      const url =
        "https://cdn.twist.vision/v2/t.resize()/playground-default.jpeg";
    ...
    it("should get obj from url with zoneslug - filedepth 1", async () => {
      const url =
        "https://cdn.twist.vision/v2/zonesl/t.resize()/playground-default.jpeg";
      const obj = urlToObj(url, {
        isCustomDomain: true,
      });
      const expectedObj = {
    ...
    it("should get obj from url - default zone url - 6 character length path segment", async () => {
      ...
      const expectedObj = {
        ...
        pattern: "original",
        filePath:
          "z0/orgs/33/skills/icons/Fynd_Platform_Commerce_Extension.png",
        version: "v2",
        worker: false,
        workerPath: "",
        options: {},
